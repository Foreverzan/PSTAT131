...
```{r}
# original plot 
ggplot(mpg, aes(x= manufacturer)) + geom_bar(stat = "count", fill="blue")
```
...
mpg %>% 
	...
	ggplot(aes(y = reorder(manufacturer,(count)), x = count)) + 
		geom_bar(stat = 'identity',fill="blue" ) + xlab("weight") +ylab("manufacturer")
```
_As we can see, the dodge produced the most cars and lincoln produced the least cars ._

# Exercise 4:
Make a box plot of hwy, grouped by cyl. Do you see a pattern? If so, what?

```{r}
ggplot(mpg, aes(y=hwy, x=factor(cyl))) + geom_boxplot(fill="green") + xlab("cyl")
```
